#!/usr/bin/env python3
"""
Move Máquinas — P3.2b: Uniformiza priceValidUntil em Offer nodes
Padrão: "priceValidUntil": "2027-12-31" (31 dez do ano+1)
...
def process_file(filepath: str) -> tuple[bool, str]:
    """Processa arquivo HTML, uniformizando priceValidUntil em Offer nodes"""
    try:
        ...
        # Calcula data de validade (31 dez do próximo ano)
        current_year = datetime.now().year
        valid_until = f"{current_year + 1}-12-31"

        # Encontra Offer nodes e uniformiza priceValidUntil
        changes_made = 0

        def update_offers(node):
            nonlocal changes_made
            if node.get('@type') == 'Offer':
                if 'priceValidUntil' not in node:
                    node['priceValidUntil'] = valid_until
                    changes_made += 1
                elif node.get('priceValidUntil') != valid_until:
                    node['priceValidUntil'] = valid_until
                    changes_made += 1
                ...
                for offer in node['offers']:
                    if isinstance(offer, dict) and offer.get('@type') == 'Offer':
                        if 'priceValidUntil' not in offer:
                            ...
                            changes_made += 1
                        elif offer.get('priceValidUntil') != valid_until:
                            offer['priceValidUntil'] = valid_until
                            changes_made += 1
        ...
        if changes_made == 0:
            return False, f"No Offer found or all already have priceValidUntil={valid_until}"
        ...
        return True, f"Uniformized priceValidUntil in {changes_made} Offer(s) → {valid_until}"
...
def main():
    ...
    for filepath in sorted(html_files):
        ...
        if success:
            ...
            success_count += 1
        elif "already have" in msg.lower():
            skipped_count += 1
        else:
            print(f"❌ {relative_path}: {msg}")
            error_count += 1
    ...
    print(f"  💰 Total Offer priceValidUntil uniformizados: {total_offers}")
    print(f"  ⏭️  Páginas já conformes: {skipped_count}")
    print(f"  ❌ Erros: {error_count}")
    print(f"  📊 Total páginas: {len(html_files)}")
    print(f"\n🎯 Impacto esperado: +1.5pp conformidade, consistência temporal")
